...
const TOPSHP_CODES: Record<string, string> = {
  ...
  '8': 'Upright cross',
  '9': 'Cube, point up',
  '10': 'Two cones point to point',
  '11': 'Two cones base to base',
  '12': 'Rhombus',
  '13': 'Two cones point up',
  '14': 'Two cones point down',
  '33': 'Flag'
};
...
export class S57PopoverComponent implements OnChanges {
  ...
  private formatValue(key: string, val: unknown): string {
    if (DEPTH_KEYS.has(key)) {
      ...
      return this.app.formatValueForDisplay(n, 'm', { category: 'depth' });
    }
    if (HEIGHT_KEYS.has(key)) {
      const n = parseFloat(String(val));
      if (isNaN(n)) return String(val);
      return this.app.formatValueForDisplay(n, 'm', { category: 'length' });
    }
    if (key === 'SIGPER') {
      const n = parseFloat(String(val));
